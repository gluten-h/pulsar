...
int CALLBACK	WinMain(HINSTANCE h_instance, HINSTANCE h_prev_instance, LPSTR lp_cmd_line, int n_cmd_show)
{
	...
	pulsar::perspective_camera camera(XMMatrixPerspectiveFovLH(pulsar::deg2rad(80.0f), (float)1280.0f / (float)720.0f, 0.001f, 1000.0f));
	{
		...
		camera_node.get_component<pulsar::transform_component>().transform.set_matrix(XMMatrixLookAtLH(camera_pos, camera_forward, camera_up));
	...
	pulsar::material brick_mat;
	{
		...
		sphere.get_component<pulsar::transform_component>().transform.set_matrix(XMMatrixScaling(0.5f, 0.5f, 0.5f)
																				 * XMMatrixRotationRollPitchYaw(0.0f, 0.0f, 0.0f)
																				 * XMMatrixTranslation(0.0f, 0.0f, 1.75));
	...
	pulsar::material metal_mat;
	{
		...
		sphere1.get_component<pulsar::transform_component>().transform.set_matrix(XMMatrixScaling(0.5f, 0.5f, 0.5f)
																				 * XMMatrixRotationRollPitchYaw(0.0f, 0.0f, 0.0f)
																				 * XMMatrixTranslation(1.25f, 0.0f, 1.75));
	...
	pulsar::material wood_mat;
	{
		...
		sphere2.get_component<pulsar::transform_component>().transform.set_matrix(XMMatrixScaling(0.5f, 0.5f, 0.5f)
																				  * XMMatrixRotationRollPitchYaw(0.0f, 0.0f, 0.0f)
																				  * XMMatrixTranslation(-1.25f, 0.0f, 1.75));
