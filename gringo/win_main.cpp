...
int CALLBACK		WinMain(HINSTANCE h_instance, HINSTANCE h_prev_instance, LPSTR lp_cmd_line, int n_cmd_show)
{
	...
	int win0 = GRNG_WM::create_window(__T("gringo"), WS_VISIBLE | WS_CAPTION | WS_MAXIMIZEBOX | WS_MINIMIZEBOX | WS_SYSMENU, CW_USEDEFAULT, CW_USEDEFAULT, 1280, 720, NULL, NULL);


	GRNG_CAMERA *camera = GRNG_CAMERA::create();
	...
	GRNG_SCENE *scene = GRNG_SCENE::create();
	GRNG_WM::set_scene(win0, scene);

	GRNG_SKYBOX_MATERIAL *skybox_mat = GRNG_SKYBOX_MATERIAL::create();
	skybox_mat->env_map.set(L"resources/cubemaps/cm00/cm00.dds");
	scene->set_skybox_material(skybox_mat);


	GRNG_VERT_SHADER *gb_pass_vs = GRNG_VERT_SHADER::create();
	GRNG_GEOM_SHADER *gb_pass_gs = GRNG_GEOM_SHADER::create();
	GRNG_FRAG_SHADER *gb_pass_fs = GRNG_FRAG_SHADER::create();
	...
	GRNG_OBJECT *cube_obj = GRNG_OBJECT::create();
	GRNG_OBJECT *sph_obj  = GRNG_OBJECT::create();
	GRNG_OBJECT *sph1_obj = GRNG_OBJECT::create();
	GRNG_OBJECT *sph2_obj = GRNG_OBJECT::create();
	...
	GRNG_MATERIAL *cube_mat = GRNG_MATERIAL::create();
	...
	GRNG_MATERIAL *sph_mat = GRNG_MATERIAL::create();
	...
	GRNG_MATERIAL *sph1_mat = GRNG_MATERIAL::create();
	...
	GRNG_MATERIAL *sph2_mat = GRNG_MATERIAL::create();
	...
	GRNG_POINT_LIGHT *pl1 = GRNG_POINT_LIGHT::create();
	GRNG_DIR_LIGHT *dl1 = GRNG_DIR_LIGHT::create();
	scene->add_light(pl1);
	scene->add_light(dl1);
	...
	GRNG_INPUT_LAYOUT *input_layout = GRNG_INPUT_LAYOUT::create();
	...
	GRNG_CAMERA *cam1 = GRNG_CAMERA::create();
	cam1->destroy();
