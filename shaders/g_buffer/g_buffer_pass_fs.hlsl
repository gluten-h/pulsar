...
fs_out       frag(float4 pos : SV_POSITION, float4 world_pos : POSITION, float3 normal : NORMAL, float2 uv : UV, float3 tangent : TANGENT0, float3 bitangent : TANGENT1)
{
	...
	float exposure = mat_exposure_map.Sample(smpl, uv).x * mat_exposure;
	...
	output.rmae = float4(mat_roughness_map.Sample(smpl, uv).x * mat_roughness, mat_metalness_map.Sample(smpl, uv).x * mat_metalness, mat_ao_map.Sample(smpl, uv).x * mat_ao, exposure);
