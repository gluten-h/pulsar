...
#include "ecs/ecs_entity_bin_tree_array.h"
...
namespace PULSAR
{
	namespace ecs
	{
		...
		private:
			...
			PULSAR::ecs::entity_bin_tree_array<MAX_COMPONENTS_TYPES> m_entity_tree;
		...
		public:
			registry()
			{
				//this->m_entities_signatures.reserve(PREALLOCATED_ENTITIES_SIZE);
				//this->m_signatures_to_entities.reserve(PREALLOCATED_SIGNATURES_SIZE);
			}
			~registry() = default;
			...
			void	destroy(PULSAR::ecs::entity entity)
			{
			...
			T &emplace(PULSAR::ecs::entity entity, Args&&... args)		// not sure about &&, google it later
			{
			...
			template <typename T>
			void	erase(PULSAR::ecs::entity entity)
			{
				const PULSAR::ecs::component_type_id component_type_id = PULSAR::ecs::component_type::id<T>();
				assert(("Component type id overflow", component_type_id < MAX_COMPONENTS_TYPES));
				assert(("Entity id is invalid", this->m_entities_signatures.find(entity) != this->m_entities_signatures.end()));
			...
			template <typename T>
			bool	is_exists(PULSAR::ecs::entity entity)
			{
				const PULSAR::ecs::component_type_id component_type_id = PULSAR::ecs::component_type::id<T>();
				assert(("Component type id overflow", component_type_id < MAX_COMPONENTS_TYPES));
				assert(("Entity id is invalid", this->m_entities_signatures.find(entity) != this->m_entities_signatures.end()));
			...
			template <typename T>
			T &get(PULSAR::ecs::entity entity)
			{
				const PULSAR::ecs::component_type_id component_type_id = PULSAR::ecs::component_type::id<T>();
				assert(("Component type id overflow", component_type_id < MAX_COMPONENTS_TYPES));
				assert(("Entity id is invalid", this->m_entities_signatures.find(entity) != this->m_entities_signatures.end()));
