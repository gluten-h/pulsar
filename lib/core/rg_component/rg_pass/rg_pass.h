...
namespace PULSAR
{
	...
	public:
		...
		void	register_input(PULSAR::RG_INPUT *input)
		{
			if (!input)
				THROW_RG_EXC("RG_PASS \"" + this->get_name() + "\": RG_INPUT cannot be NULL");

			const std::string input_name = input->get_name();
			try
			{
				this->inputs.at(input_name);
				THROW_RG_EXC("RG_PASS \"" + this->name + "\": RG_INPUT \"" + input_name + "\" already exists");
		...
		{
			if (!source)
				THROW_RG_EXC("RG_PASS \"" + this->get_name() + "\": RG_SOURCE cannot be NULL");

			const std::string source_name = source->get_name();
			try
			{
				this->sources.at(source_name);
				THROW_RG_EXC("RG_PASS \"" + this->name + "\": RG_SOURCE \"" + source_name + "\" already exists");
		...
		PULSAR::RG_INPUT	*get_input(const std::string &name)
		{
			...
			{
				THROW_RG_EXC("RG_PASS \"" + this->name + "\": RG_INPUT \"" + name + "\" not found");
			}
		}
		PULSAR::RG_SOURCE	*get_source(const std::string &name)
		{
			...
			{
				THROW_RG_EXC("RG_PASS \"" + this->name + "\": RG_SOURCE \"" + name + "\" not found");
			}
		}
		...
		void	set_input_linkage(const std::string &input_name, const std::string &source)
		{
			...
			if (source_split.size() != 2u)
				THROW_RG_EXC("RG_PASS \"" + this->name + "\": RG_INPUT source name has invalid format");
