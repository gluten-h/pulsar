...
namespace PULSAR
{
	...
	private:
		...
		PULSAR::RG_BUFFER_SOURCE<PULSAR::RENDER_TARGET>			*backbuffer_source = NULL;
		PULSAR::RG_BUFFER_SOURCE<PULSAR::DEPTH_STENCIL_VIEW>	*ds_view_source = NULL;
		...
		void		link_pass(PULSAR::RG_PASS *pass)
		{
			for (auto &it : pass->get_inputs())
			{
				...
				if (source_pass_name.empty())
					THROW_RG_EXC("RENDER_GRAPH: RG_PASS \"" + pass->get_name() + "\": RG_INPUT \"" + it.second->get_name() + "\": Source pass name is undefined");
				if (source_name.empty())
					THROW_RG_EXC("RENDER_GRAPH: RG_PASS \"" + pass->get_name() + "\": RG_INPUT \"" + it.second->get_name() + "\": Source name is undefined");

				if (source_pass_name == PULSAR::RG_ROOT_STR)
				{
					try
					{
					...
					{
						THROW_RG_EXC("RENDER_GRAPH: RG_PASS \"" + pass->get_name() + "\": RG_INPUT \"" + it.second->get_name() + "\": Global source \"" + source_name + "\" not found");
					}
				}
				else
				{
					...
					{
						THROW_RG_EXC("RENDER_GRAPH: RG_PASS \"" + pass->get_name() + "\": RG_INPUT \"" + it.second->get_name() + "\": Source pass \"" + source_pass_name + "\" not found");
					}
				}
			}
		}
	...
	public:
		RENDER_GRAPH(PULSAR::RENDER_TARGET *backbuffer, PULSAR::DEPTH_STENCIL_VIEW *depth_stencil_view) : PULSAR::RG_COMPONENT()
		{
			...
			this->backbuffer_source = PULSAR::RG_BUFFER_SOURCE<RENDER_TARGET>::create("backbuffer", this->backbuffer);
			this->ds_view_source = PULSAR::RG_BUFFER_SOURCE<DEPTH_STENCIL_VIEW>::create("depth_stencil_view", this->ds_view);
		...
		void		add_pass(PULSAR::RG_PASS *pass, unsigned int dependency_level)
		{
			if (!pass)
